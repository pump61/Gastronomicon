...
public class Gastronomicon extends AbstractAddon {
    ...
    public void enable() {
        ...
        if (!getServer().getPluginManager().isPluginEnabled("GuizhanLibPlugin")) {
            getLogger().log(Level.SEVERE, "This plugin requires the GuizhanLib dependency plugin (GuizhanLibPlugin) to run!");
            getLogger().log(Level.SEVERE, "Download it here: https://50L.cc/gzlib");
        ...
        getLogger().info("#   Gourmet   Localized by the Slimefun CN L10n Team  #");
        getLogger().info("#======================================#");
        ...
        if (isPluginEnabled("SlimeHUD")) {
            try {
                info("Detected that the server has SlimeHUD installed!");
                info("Enabling integration...");
                SlimeHUDSetup.setup();
            } catch (Exception e) {
                warn("The installed SlimeHUD version is incompatible");
                warn("Please update SlimeHUD to the latest version!");
            }
        }

        if (isPluginEnabled("DynaTech") && !getConfig().getBoolean("disable-dynatech-integration")) {
            try {
                info("Detected that the server has DynaTech installed!");
                info("Adding related crops to DynaTech...");
                DynaTechSetup.setup();
            } catch (Exception e) {
                warn("The installed DynaTech version is incompatible");
                warn("Please update DynaTech to the latest version!");
            }
        }
    ...
    public static void sendMessage(Player player, String message) {
        player.sendMessage(/* ChatColor.of("#c91df4") + "§lGourmet§7§l> §7" + */ StringUtil.formatColors(message));
    }
