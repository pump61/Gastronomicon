...
public abstract class GastroWorkstation extends MenuBlock {
    ...
    public GastroWorkstation(SlimefunItemStack item, ItemStack[] recipe) {
        this(GastroGroups.BASIC_MACHINES, item, recipe);
    }

    public GastroWorkstation(SubGroup category, SlimefunItemStack item, ItemStack[] recipe) {
     ...
     * {@link RecipeBookMenu} listing every recipe of this workstation's
     * {@link GastroRecipeType}, with a button to return to this machine.
     */
    protected void addRecipeBookButton(BlockMenuPreset preset, int slot, String title) {
        preset.addItem(slot, RECIPE_BOOK_ITEM, (p, s, item, action) -> {
            final BlockMenu menu = (BlockMenu) p.getOpenInventory().getTopInventory().getHolder();
            RecipeBookMenu.open(p, getGastroRecipeType(), title, menu.getLocation());
            return false;
        });
    }
