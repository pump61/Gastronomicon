...
public class GastroCommandExecutor implements CommandExecutor {
                    ...
                    case "proficiency":
                        if (!Gastronomicon.checkPermission(
                            ...
                            GastroTheme.PERFECT_FOOD.getColor()
                                + "§lGourmet&7> &cYou do not have permission to modify profiles!"))
                            return true;
                        ...
                        try {
                            ...
                            Gastronomicon.sendMessage(player, "&cAmount must be a positive integer!");
                            return true;
                        }
                        if (amount < 0) {
                            Gastronomicon.sendMessage(player, "&cAmount must be a positive integer!");
    ...
    private void sendProficiencies(Player player, Player toCheck) {
        Gastronomicon.sendMessage(player, Gastronomicon.getInstance().getPlayerData()
            .get(player.getUniqueId() + ".proficiencies", "A small error occurred!").toString());
    }

    private void sendInfo(Player player) {
        player.sendMessage(
            "",
            GastroTheme.PERFECT_FOOD.getColor() + "§lGourmet §8- §7Version "
    ...
    private void sendHelp(Player player) {
        Gastronomicon.sendMessage(player, "Help command not yet implemented! " +
            "Please visit the plugin repository https://schn.pages.dev/gastronomicon");
    }

    private void sendCredits(Player player) {
        player.sendMessage(
            GastroTheme.PERFECT_FOOD.getColor() + "§lGourmet §8- §7Credits:",
            "§f------",
            "§7Some head textures come from https://minecraft-heads.com/ and https://headdb.org/");
    }
    ...
    private boolean commandProfile(Player player) {
        if (!Gastronomicon.checkPermission(
            ...
            GastroTheme.PERFECT_FOOD.getColor()
                + "§lGourmet&7> &cYou do not have permission to check your profile!"))
            return true;
        Gastronomicon.sendMessage(player, "Proficiency:");
    ...
    private boolean commandProfile(Player player, Player other) {
        if (!Gastronomicon.checkPermission(
            ...
            GastroTheme.PERFECT_FOOD.getColor()
                + "§lGourmet&7> &cYou do not have permission to view other players' profiles!"))
            return true;

        Gastronomicon.sendMessage(player, other.getName() + "'s Proficiency:");
    ...
    private boolean commandProficiencyGet(Player player, String foodId) {
        if (!Gastronomicon.checkPermission(
            ...
            "gastronomicon.checkprofile",
            GastroTheme.PERFECT_FOOD.getColor()
                + "§lGourmet&7> &cYou do not have permission to modify your profile!"))
            return true;
    ...
    private boolean commandProficiencyGet(Player player, Player other, String foodId) {
        if (!Gastronomicon.checkPermission(
            ...
            "gastronomicon.checkotherprofile",
            GastroTheme.PERFECT_FOOD.getColor()
                + "§lGourmet&7> &cYou do not have permission to modify other players' profiles!"))
            return true;

        if (!GastroFood.getGastroFoodIds().stream().filter(str -> !str.startsWith("GN_PERFECT"))
            .anyMatch(str -> str.equals(foodId))) {
            Gastronomicon.sendMessage(player, "&cUnknown command!");
            return true;
        }
        final AddonConfig playerData = Gastronomicon.getInstance().getPlayerData();
        ...
        switch (mode) {
            case "set":
                ...
                Gastronomicon.sendMessage(player,
                    "Successfully set player " + other.getName() + "'s proficiency for food " + foodId + " to: " + amount);
                break;
            case "add":
                playerData.set(proficiencyPath, playerData.getInt(proficiencyPath, 0) + amount);
                Gastronomicon.sendMessage(player,
                    "Successfully added proficiency for player " + other.getName() + "'s food " + foodId + ": " + amount);
                break;
            case "sub":
                playerData.set(proficiencyPath,
                    Math.min(playerData.getInt(proficiencyPath, 0) - amount, 0));
                Gastronomicon.sendMessage(player,
                    "Successfully reduced proficiency for player " + other.getName() + "'s food " + foodId + ": " + amount);
                break;
