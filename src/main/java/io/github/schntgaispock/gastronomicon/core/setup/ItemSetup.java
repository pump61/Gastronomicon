...
import io.github.schntgaispock.gastronomicon.core.slimefun.items.food.ToastedBread;
...
import io.github.schntgaispock.gastronomicon.core.slimefun.items.workstations.manual.CuttingBoard;
import io.github.schntgaispock.gastronomicon.core.slimefun.items.workstations.manual.Freezer;
import io.github.schntgaispock.gastronomicon.core.slimefun.items.workstations.manual.StorageBlock;
import io.github.schntgaispock.gastronomicon.core.slimefun.items.workstations.manual.WaterSink;
...
public class ItemSetup {
    ...
    // rest of the Counter variants from Tag.SLABS.
    private static final Set<Material> COUNTER_VARIANT_EXCLUDED_SLABS = Set.of(
        Material.POLISHED_ANDESITE_SLAB, Material.PETRIFIED_OAK_SLAB);
    ...
    // Cabinets use the full-block equivalent of a slab as their icon/placed
    // appearance (e.g. Polished Andesite Slab -> Polished Andesite), matching
    // how the original Counter/Cabinet pair looked. Falls back to the slab
    // itself if no matching full block can be found.
    private static Material resolveFullBlockMaterial(Material slab) {
        final String base = slab.name().substring(0, slab.name().length() - "_SLAB".length());
        for (final String candidate : new String[] { base + "_PLANKS", base, base + "_BLOCK", base + "S" }) {
            final Material match = Material.matchMaterial(candidate);
            if (match != null && match.isBlock()) {
                return match;
            }
        }
        return slab;
    ...
    // Barrel in between. The Chest recipe is the item's "real" Slimefun recipe;
    // the Barrel recipe is added straight to the Enhanced Crafting Table
    // multiblock afterwards so both containers work for the same output.
    private static void registerCounter(Gastronomicon gn, SlimefunItemStack item, Material slabMaterial) {
        final ItemStack slab = new ItemStack(slabMaterial);
        ...
        new StorageBlock(
            ...
            new ItemStack[] {
                null, null, null,
                slab, new ItemStack(Material.CHEST), slab,
                null, null, null
            },
            3).register(gn);

        final SlimefunItem enhancedCraftingTable = SlimefunItem.getById("ENHANCED_CRAFTING_TABLE");
        if (enhancedCraftingTable instanceof MultiBlockMachine multiBlockMachine) {
            multiBlockMachine.addRecipe(
                new ItemStack[] {
                    null, null, null,
                    slab, new ItemStack(Material.BARREL), slab,
                    null, null, null
                },
                item.clone());
        }
    ...
    private static void registerStorageDecoVariants(Gastronomicon gn, ItemStack oakPlanks, ItemStack ironNugget) {
        for (final Material slab : Tag.SLABS.getValues()) {
            ...
            final String idSuffix = slab.name();

            final SlimefunItemStack counter = ThemedItemStack.of(
    ...
    public static void setup() {
        ...
        final ItemStack ORANGE = GastroStacks.ORANGE;
        final ItemStack LETTUCE = GastroStacks.LETTUCE;
        final ItemStack GARLIC = GastroStacks.GARLIC;
        final ItemStack TOMATO = GastroStacks.TOMATO;
        final ItemStack ONION = GastroStacks.ONION;
        final ItemStack BACON = GastroStacks.BACON;
        final ItemStack MAYO = GastroStacks.MAYO;
        final ItemStack CORN = GastroStacks.CORN;
        final ItemStack BBQ_SAUCE = GastroStacks.BBQ_SAUCE;
        final ItemStack CURRY_LEAF = GastroStacks.CURRY_LEAF;
        final ItemStack CILANTRO = GastroStacks.CILANTRO;
        final ItemStack BLUEBERRY = GastroStacks.BLUEBERRY;
        final ItemStack CRANBERRY = GastroStacks.CRANBERRY;
        final ItemStack GRAPE = GastroStacks.GRAPE;
        final ItemStack TEA_LEAF = GastroStacks.TEA_LEAF;
        final ItemStack STRAWBERRY = GastroStacks.STRAWBERRY;
        final ItemStack COWBERRY = GastroStacks.COWBERRY;
        final ItemStack COCONUT = GastroStacks.COCONUT;
        final ItemStack CHERRY = GastroStacks.CHERRY;
        final ItemStack RASPBERRY = GastroStacks.RASPBERRY;
        final ItemStack PEACH = GastroStacks.PEACH;
        final ItemStack PINEAPPLE = GastroStacks.PINEAPPLE;
        final ItemStack LEMON = GastroStacks.LEMON;
        final ItemStack YEAST = GastroStacks.YEAST;
        final ItemStack BROWN_SUGAR = GastroStacks.BROWN_SUGAR;
        ...
        new CuttingBoard(
            ...
            new ItemStack[] {
                null, null, null,
                null, null, null,
                new ItemStack(Material.SPRUCE_SLAB), new ItemStack(Material.SPRUCE_SLAB), new ItemStack(Material.SPRUCE_SLAB)
            }).register(gn);
        new UnplaceableSolid(
            GastroGroups.TOOLS,
        ...
        new HuntingTrap(
            ...
            @Override
            protected boolean canCatch(Location l) {
                return CRAB_TRAP_BIOMES.contains(l.getBlock().getBiome());
            }
        }.register(gn);
        ...
        new WaterSink(
            GastroStacks.WATER_SINK,
            new ItemStack[] {
                SlimefunItems.DAMASCUS_STEEL_INGOT, SlimefunItems.DAMASCUS_STEEL_INGOT, SlimefunItems.DAMASCUS_STEEL_INGOT,
                SlimefunItems.REINFORCED_ALLOY_INGOT, WATER_BUCKET, SlimefunItems.REINFORCED_ALLOY_INGOT,
                SlimefunItems.REINFORCED_ALLOY_INGOT, SlimefunItems.REINFORCED_ALLOY_INGOT, SlimefunItems.REINFORCED_ALLOY_INGOT
            }).register(gn);

        // Not craftable normally - dropping an anvil on a cow turns it into
        // this (see CowInAJarListener).
        new CowInAJar(
            GastroStacks.COW_IN_A_JAR,
            new ItemStack[] {
                null, null, null,
                null, GastroStacks.GUIDE_ANVIL_ON_COW, null,
                null, new ItemStack(Material.COW_SPAWN_EGG), null
            }).register(gn);
        ...
        new StorageBlock(
            GastroStacks.FRIDGE_MODULE,
            new ItemStack[] {
                IRON_BLOCK, IRON_BLOCK, IRON_BLOCK,
                IRON_BLOCK, SlimefunItems.COOLING_UNIT, IRON_BLOCK,
                SlimefunItems.BASIC_CIRCUIT_BOARD, SlimefunItems.ELECTRIC_MOTOR, IRON_BLOCK
            },
            4).register(gn);
        ...
        new GreenHouse(
            ...
            new ItemStack[] {
                ...
                new ItemStack(Material.WAXED_OXIDIZED_COPPER), SlimefunItems.BASIC_CIRCUIT_BOARD, new ItemStack(Material.WAXED_OXIDIZED_COPPER),
                new ItemStack(Material.WAXED_OXIDIZED_COPPER), SlimefunItems.ELECTRIC_MOTOR, new ItemStack(Material.WAXED_OXIDIZED_COPPER)
            }).register(gn);
        ...
        new Toaster(
            ...
            new ItemStack[] {
                SlimefunItems.STEEL_INGOT, SlimefunItems.STEEL_INGOT, SlimefunItems.STEEL_INGOT,
                SlimefunItems.STEEL_INGOT, new ItemStack(Material.SMOKER), SlimefunItems.STEEL_INGOT,
        ...
        new CoffeeMachine(
            ...
            2000, 100,
            new ItemStack[] {
                SlimefunItems.STEEL_INGOT, SlimefunItems.CRAFTING_MOTOR, SlimefunItems.STEEL_INGOT,
                SlimefunItems.STEEL_INGOT, new ItemStack(Material.GLASS), SlimefunItems.STEEL_INGOT,
                SlimefunItems.STEEL_INGOT, SlimefunItems.STEEL_PLATE, SlimefunItems.STEEL_INGOT
            }).register(gn);
        ...
        // These can no longer be planted in the world (that in-world growth
        // system kept breaking) - they're obtained only by breaking grass
        // (see WildHarvestListener) and grown exclusively in a Green House
        // (see GreenHouseCrops).
        registerGreenHouseCrop(gn, GastroStacks.TOMATO_SEEDS, GastroStacks.TOMATO);
        registerGreenHouseCrop(gn, GastroStacks.LETTUCE_SEEDS, GastroStacks.LETTUCE);
        registerGreenHouseCrop(gn, GastroStacks.GARLIC_SEEDS, GastroStacks.GARLIC);
        registerGreenHouseCrop(gn, GastroStacks.ONION_SEEDS, GastroStacks.ONION);
        registerGreenHouseCrop(gn, GastroStacks.STRAWBERRY_SEEDS, GastroStacks.STRAWBERRY);
        registerGreenHouseCrop(gn, GastroStacks.CORN_SEEDS, GastroStacks.CORN);
        registerGreenHouseCrop(gn, GastroStacks.CILANTRO_SEEDS, GastroStacks.CILANTRO);
        registerGreenHouseCrop(gn, GastroStacks.CURRY_LEAF_SEEDS, GastroStacks.CURRY_LEAF);
        registerGreenHouseCrop(gn, GastroStacks.TEA_LEAF_SEEDS, GastroStacks.TEA_LEAF);
        registerGreenHouseCrop(gn, GastroStacks.PINEAPPLE_SEEDS, GastroStacks.PINEAPPLE);
        registerGreenHouseCrop(gn, GastroStacks.GRAPE_SEEDS, GastroStacks.GRAPE);
        registerGreenHouseCrop(gn, GastroStacks.ORANGE_SEEDS, GastroStacks.ORANGE);
        registerGreenHouseCrop(gn, GastroStacks.LEMON_SEEDS, GastroStacks.LEMON);
        registerGreenHouseCrop(gn, GastroStacks.CHERRY_SEEDS, GastroStacks.CHERRY);
        registerGreenHouseCrop(gn, GastroStacks.PEACH_SEEDS, GastroStacks.PEACH);
        registerGreenHouseCrop(gn, GastroStacks.COCONUT_SEEDS, GastroStacks.COCONUT);
        registerGreenHouseCrop(gn, GastroStacks.BLUEBERRY_SEEDS, GastroStacks.BLUEBERRY);
        registerGreenHouseCrop(gn, GastroStacks.CRANBERRY_SEEDS, GastroStacks.CRANBERRY);
        registerGreenHouseCrop(gn, GastroStacks.RASPBERRY_SEEDS, GastroStacks.RASPBERRY);
        registerGreenHouseCrop(gn, GastroStacks.COWBERRY_SEEDS, GastroStacks.COWBERRY);

        // -- Processed replacements for ExoticGarden ingredients --

        new SimpleGastroFoodBuilder()
            .type(GastroRecipeType.CUTTING_BOARD)
            .item(GastroStacks.BACON)
        ...
        new SimpleGastroFoodBuilder()
            .type(GastroRecipeType.CUTTING_BOARD)
            .item(GastroStacks.SLICED_BREAD)
            .ingredients(BREAD)
            .tools(GastroStacks.KITCHEN_KNIFE)
            .register(gn);
        ...
        new SimpleGastroFoodBuilder()
            .type(GastroRecipeType.CUTTING_BOARD)
            .item(GastroStacks.SLICED_SALMON)
            .ingredients(SALMON)
            .tools(GastroStacks.KITCHEN_KNIFE)
            .register(gn);
        ...
        new SimpleGastroFoodBuilder()
            ...
            .item(GastroStacks.MAYO)
            .ingredients(EGG, MILK_BUCKET)
        ...
        new SimpleGastroFoodBuilder()
            .type(GastroRecipeType.MULTI_STOVE)
            .item(GastroStacks.BBQ_SAUCE)
            .ingredients(GastroStacks.KETCHUP, GastroStacks.BROWN_SUGAR)
            .container(GastroStacks.WATER_BOTTLE)
            .temperature(Temperature.LOW)
            .register(gn);
        ...
        new SimpleGastroFoodBuilder()
            ...
            .item(GastroStacks.YEAST)
            .ingredients(SUGAR)
            .register(gn);

        new SimpleGastroFoodBuilder()
            .type(GastroRecipeType.CULINARY_WORKBENCH)
            .item(GastroStacks.BROWN_SUGAR)
            .ingredients(SUGAR, HONEY_BOTTLE)
            .register(gn);
        ...
        new UnplaceableSolid(
            ...
            RecipeUtil.singleCenter(Material.SHORT_GRASS))
                .register(gn);
        GreenHouseCrops.register(GastroStacks.LYCHEE_SAPLING, GastroStacks.LYCHEE);
        ...
        new UnplaceableSolid(
            ...
            RecipeUtil.singleCenter(Material.SHORT_GRASS))
                .register(gn);
        GreenHouseCrops.register(GastroStacks.BANANA_SAPLING, GastroStacks.BANANA);

        registerGreenHouseCrop(gn, GastroStacks.COFFEE_SAPLING, GastroStacks.COFFEE_BEANS);
        ...
        new ToastedBread(
            GastroGroups.FOOD,
            GastroStacks.TOASTED_BREAD,
            GastroRecipeType.TOASTER,
            RecipeUtil.singleCenter(GastroStacks.SLICED_BREAD))
                .register(gn);
        ...
        RecipeRegistry.registerRecipe(new ShapelessGastroRecipe(
            GastroRecipeType.MILL,
            RecipeUtil.collection(new ItemStack(Material.SAND, 2)),
            new HashSet<>(),
            SlimefunItems.SALT));

        // Alternate Mill recipe for Slimefun's Wheat Flour, using the same
        // ingredients as its Grind Stone recipe
        RecipeRegistry.registerRecipe(new ShapelessGastroRecipe(
            GastroRecipeType.MILL,
            RecipeUtil.collection(new ItemStack(Material.WHEAT)),
            new HashSet<>(),
        ...
        // Alternate Culinary Workbench recipe for Slimefun's Heavy Cream, using
        // the same ingredients as its Enhanced Crafting Table recipe
        RecipeRegistry.registerRecipe(new ShapedGastroRecipe(
            GastroRecipeType.CULINARY_WORKBENCH,
            RecipeUtil.collection(MILK_BUCKET),
            Set.of(GastroStacks.WHISK),
            SlimefunItems.HEAVY_CREAM.asQuantity(2)));

        // Alternate Culinary Workbench recipe for Slimefun's Butter, using the
        // same ingredients as its Enhanced Crafting Table recipe
        RecipeRegistry.registerRecipe(new ShapedGastroRecipe(
            GastroRecipeType.CULINARY_WORKBENCH,
            RecipeUtil.collection(SlimefunItems.HEAVY_CREAM, SlimefunItems.SALT),
            Set.of(GastroStacks.WHISK),
        ...
        // Multi Stove recipe for vanilla Dried Kelp
        RecipeRegistry.registerRecipe(new MultiStoveRecipe(
            RecipeUtil.collection(KELP),
            new HashSet<>(),
            Temperature.LOW,
            DRIED_KELP));
        ...
        // Alternate Multi Stove recipe for Slimefun's Cheese, using the same
        // ingredients as its Enhanced Crafting Table recipe
        RecipeRegistry.registerRecipe(new MultiStoveRecipe(
            RecipeUtil.collection(MILK_BUCKET, SlimefunItems.SALT),
            Set.of(GastroStacks.WHISK),
        ...
        new GastroFoodBuilder()
            .type(GastroRecipeType.CULINARY_WORKBENCH)
            .item(GastroStacks.NIGIRI)
            .amount(3)
            .shape(RecipeShape.SHAPED)
            .ingredients(
                GastroStacks.SLICED_SALMON, GastroStacks.SLICED_SALMON, GastroStacks.SLICED_SALMON,
                GastroStacks.COOKED_RICE, GastroStacks.COOKED_RICE, GastroStacks.COOKED_RICE,
                DRIED_KELP, DRIED_KELP, DRIED_KELP)
            .tools(GastroStacks.KITCHEN_KNIFE)
            .register(gn);
    ...
    /**
     * Registers a Green House seed/sapling as a non-placeable ingredient item
     * (it can no longer be planted in the world) and registers which fruit it
     * grows into via {@link GreenHouseCrops}.
     *
     * The fruit itself must also be registered as a real SlimefunItem (not
     * just data on a SlimefunItemStack) - otherwise any recipe using it as an
     * ingredient throws a NullPointerException from
     * SlimefunItem.getById(fruit.getItemId()) returning null when that
     * recipe is checked.
     */
    private static void registerGreenHouseCrop(Gastronomicon gn, SlimefunItemStack seed, SlimefunItemStack fruit) {
        new UnplaceableSolid(GastroGroups.RAW_INGREDIENTS, seed, GastroRecipeType.BREAK,
            RecipeUtil.singleCenter(Material.SHORT_GRASS)).register(gn);
        new UnplaceableSolid(GastroGroups.RAW_INGREDIENTS, fruit, GastroRecipeType.HARVEST,
            RecipeUtil.singleCenter(seed)).register(gn);
        GreenHouseCrops.register(seed, fruit);
    }
}
