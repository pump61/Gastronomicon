...
public class RecipeBookMenu {
    ...
    private static final int DETAIL_CONTAINER_SLOT = 31;
    private static final int[] DETAIL_TOOL_SLOTS = { 37, 38, 39, 40, 41 };
    private static final int DETAIL_ARROW_SLOT = 22;
    private static final int DETAIL_OUTPUT_SLOT = 24;
    private static final int DETAIL_BACK_SLOT = 49;
    private static final int DETAIL_RETURN_TO_MACHINE_SLOT = 47;
    ...
    private static void openList(Player p, GastroRecipeType type, String title, Location machineLocation, int page) {
        ...
        if (page_ > 0) {
            menu.addItem(PREV_PAGE_SLOT, new CustomItemStack(Material.ARROW, "&ePrevious Page"),
                (pl, slot, item, action) -> {
        ...
        menu.addItem(PAGE_INFO_SLOT,
            new CustomItemStack(Material.BOOK, "&ePage " + (page_ + 1) + " / " + pageCount),
            (pl, slot, item, action) -> false);

        if (page_ < pageCount - 1) {
            menu.addItem(NEXT_PAGE_SLOT, new CustomItemStack(Material.ARROW, "&eNext Page"),
                (pl, slot, item, action) -> {
                    openList(pl, type, title, machineLocation, page_ + 1);
                    return false;
                });
    ...
    private static void openDetail(Player p, GastroRecipeType type, String title, Location machineLocation, int page,
        ...
        menu.addItem(DETAIL_ARROW_SLOT, new CustomItemStack(Material.ARROW, " "), (pl, slot, item, action) -> false);
        menu.addItem(DETAIL_OUTPUT_SLOT, recipe.getOutputs()[0].clone(), (pl, slot, item, action) -> false);

        menu.addItem(DETAIL_BACK_SLOT, new CustomItemStack(Material.BARRIER, "&cBack"), (pl, slot, item, action) -> {
    ...
    private static void addReturnButton(ChestMenu menu, int slot, Location machineLocation) {
        menu.addItem(slot, new CustomItemStack(Material.LIME_STAINED_GLASS_PANE, "&aReturn to Machine"),
            (pl, s, item, action) -> {
